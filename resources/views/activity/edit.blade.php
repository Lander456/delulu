...
<x-layout>
    <div class="flex min-h-screen h-full bg-background">
        <div class="flex-row flex-[4] min-h-screen h-full bg-background">
            <form action="{{ route('activities.update', parameters: $activity) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col  min-w-0 px-5 py-4 bg-background">
                    ...
                    <!-- Step  -->
                    <label class="font-semibold text-lg px-5 mb-2">From Step:</label>
                    <div class="px-5 mb-4">
                        <div>
                            {{ $activity->step->name }}
                        </div>
                        <input type="hidden" name="step_id" value="{{ $activity->step->id }}">
                    </div> 
                </div>
            </form>
        </div>
