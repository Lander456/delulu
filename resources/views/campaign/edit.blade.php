...
<x-layout>
    <div class="flex min-h-screen h-full bg-background">
        <div class="flex-row flex-[4] min-h-screen h-full bg-background">
            <form action="{{ route('campaigns.update', parameters: $campaign) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col  min-w-0 px-5 py-4 bg-background">
                    ...
                    <label class="font-semibold text-lg px-5 mb-2">Assigned Administrator:</label>
                    <div class="px-5 mb-4">
                            ...
                            <select name="user_id" id="user_id" 
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" class="text-black" 
                                        @selected(optional($campaign->user)->id === $user->id)>
                                        {{ $user->username }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="px-5 mb-4">
                        ...
                        <select name="current_step_id" id="current_step_id" 
                                ...
                                <option value="{{ $step->id }}" 
                                    @selected(optional($campaign->currentStep)->id === $step->id)>
                                    {{ $step->name }}
                                </option>
                            @endforeach
                        </select>
