...
@endphp
<x-slot:title>{{ $informationSource->name }}</x-slot:title>
<x-layout>
        ...
        <div class = "flex flex-col flex-[4] min-w-64 px-5 py-4  bg-background">
            <div class="flex justify-between">
                ...
                <div class="flex flex-row gap-4">
                    <!-- Edit Button -->
                    @can('update', $informationSource)
                        <div class="flex flex-col  items-center text-black justify-center">
                            <a href="{{ route('informationSources.edit', $informationSource) }}" 
                    ...
                    @can('update', $informationSource)
                        <div class="flex flex-col  items-center text-black justify-center">
                            <form action="{{ route('informationSources.destroy', $informationSource) }}" method="POST"
            ...
            <div class="flex px-5 mb-4 max-w-300 w-2/3">
                @if($informationSource->themes->isNotEmpty())
                <table class="w-full border  border-background-darker border-t-2 ">
                    ...
                    <tbody >
                        @foreach($informationSource->themes as $theme)
                        @can('view',  $theme)
                        <tr>
            ...
            <div x-data="{ open: false }" class="px-5 mb-4">
                ...
                <div class="flex mb-4 max-w-300">
                    @if($informationSource->targetDemographics->isNotEmpty())
                    <table class="w-full border  border-background-darker border-t-2">
                        <thead class="bg-background-dark">
                            <tr class="border border-background-darker border-t-2 ">
                        ...
                        <tbody >
                            @foreach($informationSource->targetDemographics as $demo)
